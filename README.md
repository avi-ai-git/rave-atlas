...
**Rave Set Builder** builds a tracklist with a deliberate energy arc. If you are new to the scene, three mood starter cards give you a one-click entry point: First Night (accessible melodic intro at Watergate), The Real Berlin (peak-time Berghain), and Sunday Morning (Sisyphos outdoor comedown). A Surprise Me button picks a random set from a curated list of 20 seeds spanning Berlin's electronic spectrum, always on genre. Experienced users can ignore all of that and describe their own slot in the chat input below. Give it a vibe, a time of night, a track count, or a venue, and it returns each track with a function role (opener, build, peak, sustain, resolution, closer), a BPM target, a one-line reason for each position, and a 30-second Deezer preview plus a YouTube link. A Last.fm genre guard rejects non-electronic artists before the catalogue call so the set stays on genre. A two-to-three sentence set story describes the energy narrative across the whole arc. Tracks are grounded in Deezer's real catalogue so the titles are always playable, and the sidebar model selection applies here too.
...
4. **The set builder.** "Build me a deep, melodic house-party set, warmer as it goes, 16 tracks." A full arc from a 2 at the open to an 8 at the peak, each track with a reason and a preview. New to the scene? Hit one of the three mood cards or Surprise Me and skip the blank-page problem entirely.
